...
function ensureBaseHistoryState() {
    ...
    if (!state || !state.__ReSuSFSUiHistory) {
        window.history.replaceState({ __ReSuSFSUiHistory: true, depth: 0 }, "");
    }

    window.addEventListener("popstate", (event) => {
        const nextDepth = event.state?.__ReSuSFSUiHistory ? event.state.depth : 0;
...
function replaceTopLayer(nextLayer) {
    ...
    if (previousLayer) {
        window.history.replaceState({ __ReSuSFSUiHistory: true, depth: 1 }, "");
    } else {
        window.history.pushState({ __ReSuSFSUiHistory: true, depth: 1 }, "");
    }
}
